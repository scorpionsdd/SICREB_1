...
public partial class PersonasMoralesGL : System.Web.UI.Page
{
    int idUs;
    //private Archivo ultimoArchivo; //  comentado previamente
    private string UltimoArchivoGL = "";
    ...
    // Constantes para el segmento TS Version de Base PM 4.0
    const string TS_NOMBRE = "SEGMENTO TS";
    const int TS_LNG = 2;
    const int TS_POS = 1;

    const string TS00_NOMBRE = "NUMERO DE COMPAÑIAS REPORTADAS";
    const int TS00_LNG = 7;
    const int TS00_POS = 5;

    const string TS01_NOMBRE = "TOTAL DE CANTIDAD (SALDO)";
    const int TS01_LNG = 30;
    const int TS01_POS = 14;

    const string TS02_NOMBRE = "FILLER";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            getFacultades();
            ActividadRules.GuardarActividad(5, this.idUs, "Acceso al Menú detalle de Personas Morales GL");
        }
        else
        {
        ...
        {
            // Revisamos si el usuario esta logueado en la aplicacion si tiene privilegios para consultar el menu actual.
            // Esto tambien nos sirve para enviar al Login una vez terminado el tiempo se Sesion.
            if (Session["Facultades"] != null)
            {
                getFacultades();
                ActividadRules.GuardarActividad(5, this.idUs, "Acceso al Menú detalle de Personas Morales GL");
            ...
            ActividadRules.GuardarActividad(5, idUs, "Acceso al Menú detalle de Personas Morales GL");

            // Obtenemos el ultimo archivo PM generado
            Archivos_Rules archRules = new Archivos_Rules(Enums.Persona.Moral);
            UltimoArchivoGL = archRules.GetUltimoArchivoGL();
            //ultimoArchivo = archRules.GetUltimoArchivo(); // comentado previamente
            
            // Si el ultimo proceso se ejecut bien y recuperamos el archivo, mostramos la informacion.
            if (UltimoArchivoGL.Trim() != string.Empty)
            {
                ...
                Session.Add("UltimoArchivoPM", UltimoArchivoGL);

                // Consultamos los Warnings y Errores generardos durante la construccion del archivo PM
                // Eliminar string strGrupos = "13,6378";
                ErroresWarnings_Rules errores = new ErroresWarnings_Rules();
                Session.Add("erroresInfoPM", errores.GetErrorWarningGL(1) );
                Session.Add("warningsInfoPM", errores.GetErrorWarningGL(0) );
            }

            CambiaAtributosRGR(rgArchivoGL.FilterMenu);
            CambiaAtributosRGR(rgErrores.FilterMenu);
            CambiaAtributosRGR(rgWarnings.FilterMenu);
        }
    }

    protected void rgArchivoGL_NeedDataSource(object source, Telerik.Web.UI.GridNeedDataSourceEventArgs e)
    {
        ...
        if (Session["UltimoArchivoPM"] != null)
        {
            ...
            string ArchivoPM = (Session["UltimoArchivoPM"] != null) ? Session["UltimoArchivoPM"] as string : string.Empty;

            PM_Cinta cinta = LoadPMCinta();
            cinta.ArchivoId = 1; //ultimoArchivo.Id;
            ...
            this.rgArchivoGL.MasterTableView.DataSource = cinta.EMs;
            this.rgArchivoGL.MasterTableView.DetailTables[0].DataSource = crs;
            this.rgArchivoGL.MasterTableView.DetailTables[1].DataSource = acs;
            this.rgArchivoGL.MasterTableView.DetailTables[2].DataSource = avs;
        }
    }
    ...
    protected void imgExportArchivoXLS_Click(object sender, ImageClickEventArgs e)
    {
        ...
        rgArchivoGL.MasterTableView.AllowPaging = false;
        rgArchivoGL.Rebind();

        foreach (GridDataItem dgi in rgArchivoGL.MasterTableView.Items)
            dgi.Expanded = true;
        ...
        titulares[0] = "titulo";
        string fileName = string.Format("{0}{2}{1}", "PersonasMoralesGLDetalles", ".xls", DateTime.Now.ToString());

        DataSet dsReportePM = ExportarExcel.GenerarExcelDetallesPM(rg, titulares, "Detalle Personas Morales GPOS y LCCyR", "Banobras");
        ExportarReportePM(dsReportePM, fileName);

        rgArchivoGL.MasterTableView.AllowPaging = true;
        rgArchivoGL.Rebind();
    }
    ...
    protected void imgExportArchivoPDF_Click(object sender, ImageClickEventArgs e)
    {
        ...
        string[] nombres = new string[] { "PROCESO DE PERSONAS MORALES GL" };
        List<RadGrid> listaGrids = new List<RadGrid>();

        try
        {
            Response.ContentType = "application/force-download";
            rgArchivoGL.MasterTableView.HierarchyDefaultExpanded = true;
            rgArchivoGL.ExportSettings.OpenInNewWindow = false;
            rgArchivoGL.ExportSettings.ExportOnlyData = true;
            rgArchivoGL.ExportSettings.IgnorePaging = true;
            rgArchivoGL.ExportSettings.OpenInNewWindow = true;
            rgArchivoGL.ExportSettings.Pdf.PageHeight = Unit.Parse("162mm");
            rgArchivoGL.ExportSettings.Pdf.PageWidth = Unit.Parse("600mm");
            rgArchivoGL.MasterTableView.ExportToPdf();
